...
public final class Application {
    ...
    private static int printUsage() {
        ...
        System.out.println("nbbiometrics-identify-console-sample-java <template count> <template filename 1> ... <template filename N> <command> (optional:<command options>)");
        System.out.println("\t<template type> - format of template to be identified (proprietary, iso, ansi, isocc)");
        System.out.println("\t<template count> - template count (N)");
        System.out.println("\t<template filename 1 .. N> - filenames of gallery templates (live fingerprint will be matched against)");
    ...
    private static NBBiometricsTemplate loadTemplate(NBBiometricsContext context, NBBiometricsTemplateType templateType, String fileName) throws IOException {
        System.out.println(String.format("Reading template from file %s (template type %s)...", fileName, templateType.toString()));
        NBBiometricsTemplate template = context.loadTemplate(templateType, readAllBytes(fileName));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            int templateCount = Integer.parseInt(args[1]);
            NBBiometricsFingerPosition fingerPosition = NBBiometricsFingerPosition.UNKNOWN;
            if (args.length < 2 + templateCount) {
                printUsage();
                return;
            }

            SampleOptions options = parseCommandOptions(Arrays.copyOfRange(args, 2 + templateCount, args.length));
	    ...
	    //Scan-Format and Device Calibration Support
            ...
            NBBiometricsSecurityLevel securityLevel = NBBiometricsSecurityLevel.NORMAL;
            int securityLevelValue = context.getSecurityLevel(securityLevel);
            System.out.println(String.format("Security level (for %s security level): %d", securityLevel.toString(), securityLevelValue));

            System.out.println("Loading templates ...");
            List<AbstractMap.SimpleEntry<Object, NBBiometricsTemplate>> templates = new LinkedList<>();

            for (int i = 0; i < templateCount; i++) {
                String templatePath = args[i + 2];
                templates.add(new AbstractMap.SimpleEntry<Object, NBBiometricsTemplate>(templatePath, loadTemplate(context, templateType, templatePath)));
            }

            System.out.println("Identification started, please put your fingerprint on sensor!");
            NBBiometricsIdentifyResult result = context.identify(templateType, fingerPosition, scanFormatInfo, NBDevice.SCAN_TIMEOUT_INFINITE, new ScanPreview(),
                    templates.iterator(), securityLevelValue);
            if (result.getStatus() == NBBiometricsStatus.OK) {
                System.out.println(String.format("Identification successful, identified as %s, score: %d", result.getTemplateId(), result.getScore()));
            } else {
                System.out.println("Not identified, status: " + result.getStatus());
            }
